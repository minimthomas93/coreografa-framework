...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['regex_match']['memory_peak'] >= 252446 or time_and_memory['regex_match']['memory_peak'] <= 503373 or time_and_memory['re_match']['memory_peak'] >= 252446 or (time_and_memory['re_match']['memory_peak'] <= 503373))
    return all(results)
...
where count_pattern_tokens(<start>) >= 29.0
