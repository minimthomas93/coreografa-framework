...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['regex_match']['runtime_ns'] >= 312771660 or time_and_memory['regex_match']['runtime_ns'] <= 347486900 or time_and_memory['re_match']['runtime_ns'] >= 312771660 or (time_and_memory['re_match']['runtime_ns'] <= 347486900))
    return all(results)
...
where count_pattern_tokens(<start>) >= 37.0
where count_pattern_tokens(<start>) <= 37.0
