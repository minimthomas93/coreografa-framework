...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['regex_match']['runtime_ns'] >= 334500 or time_and_memory['regex_match']['runtime_ns'] <= 40591000 or time_and_memory['re_match']['runtime_ns'] >= 334500 or (time_and_memory['re_match']['runtime_ns'] <= 40591000))
    return all(results)
...
where count_pattern_tokens(<start>) >= 1.0
