...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['regex_match']['runtime_ns'] >= 18481760 or time_and_memory['regex_match']['runtime_ns'] <= 20774780 or time_and_memory['re_match']['runtime_ns'] >= 18481760 or (time_and_memory['re_match']['runtime_ns'] <= 20774780))
    return all(results)
...
where count_term(<start>) >= 17.0
where count_term(<start>) <= 19.0
