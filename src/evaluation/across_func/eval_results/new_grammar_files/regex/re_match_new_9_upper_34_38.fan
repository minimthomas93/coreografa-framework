...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['regex_match']['memory_peak'] >= 4464 or time_and_memory['regex_match']['memory_peak'] <= 7836 or time_and_memory['re_match']['memory_peak'] >= 4464 or (time_and_memory['re_match']['memory_peak'] <= 7836))
    return all(results)
