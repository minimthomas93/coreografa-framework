...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['regex_match']['memory_peak'] >= 1520 or time_and_memory['regex_match']['memory_peak'] <= 6878 or time_and_memory['re_match']['memory_peak'] >= 1520 or (time_and_memory['re_match']['memory_peak'] <= 6878))
    return all(results)
