...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['regex_match']['runtime_ns'] >= 69764980 or time_and_memory['regex_match']['runtime_ns'] <= 104480220 or time_and_memory['re_match']['runtime_ns'] >= 69764980 or (time_and_memory['re_match']['runtime_ns'] <= 104480220))
    return all(results)
...
where count_pattern_tokens(<start>) >= 9.0
where count_pattern_tokens(<start>) <= 35.0
