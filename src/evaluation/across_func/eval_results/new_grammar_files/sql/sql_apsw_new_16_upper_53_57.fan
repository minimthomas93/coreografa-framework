...
<column_list> ::= (<column_name> {53,57} | <column_name> ',' <column_list>)
...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['runtime_ns'] >= 6342700 or time_and_memory['sql_apsw']['runtime_ns'] <= 9127200 or time_and_memory['sql_sqlite']['runtime_ns'] >= 6342700 or (time_and_memory['sql_sqlite']['runtime_ns'] <= 9127200))
    return all(results)
