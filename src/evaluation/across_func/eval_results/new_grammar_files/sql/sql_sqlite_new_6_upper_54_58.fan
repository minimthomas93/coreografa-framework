...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['memory_peak'] >= 2405 or time_and_memory['sql_apsw']['memory_peak'] <= 4754 or time_and_memory['sql_sqlite']['memory_peak'] >= 2405 or (time_and_memory['sql_sqlite']['memory_peak'] <= 4754))
    return all(results)
