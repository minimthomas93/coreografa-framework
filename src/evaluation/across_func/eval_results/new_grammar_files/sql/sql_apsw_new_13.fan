...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['runtime_ns'] >= 136996690 or time_and_memory['sql_apsw']['runtime_ns'] <= 152104800 or time_and_memory['sql_sqlite']['runtime_ns'] >= 136996690 or (time_and_memory['sql_sqlite']['runtime_ns'] <= 152104800))
    return all(results)
...
where count_column_name(<start>) >= 50.0
where count_column_name(<start>) <= 51.0
