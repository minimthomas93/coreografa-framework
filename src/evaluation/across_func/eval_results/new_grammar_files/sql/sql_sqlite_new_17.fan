...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['runtime_ns'] >= 10899510 or time_and_memory['sql_apsw']['runtime_ns'] <= 12340940 or time_and_memory['sql_sqlite']['runtime_ns'] >= 10899510 or (time_and_memory['sql_sqlite']['runtime_ns'] <= 12340940))
    return all(results)
...
where count_column_name(<start>) >= 54.0
where count_column_name(<start>) <= 54.0
