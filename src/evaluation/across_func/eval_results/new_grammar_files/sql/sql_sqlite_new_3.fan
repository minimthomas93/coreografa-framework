...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['runtime_ns'] >= 9458080 or time_and_memory['sql_apsw']['runtime_ns'] <= 10899510 or time_and_memory['sql_sqlite']['runtime_ns'] >= 9458080 or (time_and_memory['sql_sqlite']['runtime_ns'] <= 10899510))
    return all(results)
...
where count_column_name(<start>) >= 48.0
where count_column_name(<start>) <= 57.0
