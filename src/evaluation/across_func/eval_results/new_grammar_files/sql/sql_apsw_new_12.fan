...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['memory_peak'] >= 4063 or time_and_memory['sql_apsw']['memory_peak'] <= 4165 or time_and_memory['sql_sqlite']['memory_peak'] >= 4063 or (time_and_memory['sql_sqlite']['memory_peak'] <= 4165))
    return all(results)
...
where count_column_name(<start>) >= 0.0
where count_column_name(<start>) <= 13.0
