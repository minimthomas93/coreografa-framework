...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['memory_peak'] >= 4674 or time_and_memory['sql_apsw']['memory_peak'] <= 4776 or time_and_memory['sql_sqlite']['memory_peak'] >= 4674 or (time_and_memory['sql_sqlite']['memory_peak'] <= 4776))
    return all(results)
...
where count_column_name(<start>) >= 55.0
where count_column_name(<start>) <= 63.0
