...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['sql_apsw']['memory_peak'] >= 4094 or time_and_memory['sql_apsw']['memory_peak'] <= 4424 or time_and_memory['sql_sqlite']['memory_peak'] >= 4094 or (time_and_memory['sql_sqlite']['memory_peak'] <= 4424))
    return all(results)
...
where count_column_name(<start>) >= 49.0
where count_column_name(<start>) <= 65.0
