...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['redos_match']['runtime_ns'] >= 545000 or time_and_memory['redos_match']['runtime_ns'] <= 545000 or time_and_memory['not_redos_match']['runtime_ns'] >= 545000 or (time_and_memory['not_redos_match']['runtime_ns'] <= 545000))
    return all(results)

def count_a(s: str) -> int:
    return s.count('a')
where constraints(<start>)
where count_a(<start>) >= 1.0
