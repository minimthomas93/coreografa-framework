...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['redos_match']['memory_peak'] >= 3627 or time_and_memory['redos_match']['memory_peak'] <= 3654 or time_and_memory['not_redos_match']['memory_peak'] >= 3627 or (time_and_memory['not_redos_match']['memory_peak'] <= 3654))
    return all(results)
