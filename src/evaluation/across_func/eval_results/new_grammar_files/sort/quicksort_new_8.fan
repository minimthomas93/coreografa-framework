...
def constraints(tree):
    ...
    for func_name in user_system_funcs:
        ...
        results.append(time_and_memory['quicksort']['memory_peak'] >= 284 or time_and_memory['quicksort']['memory_peak'] <= 476 or time_and_memory['bubble_sort']['memory_peak'] >= 284 or (time_and_memory['bubble_sort']['memory_peak'] <= 476) or (time_and_memory['insertionSort']['memory_peak'] >= 284) or (time_and_memory['insertionSort']['memory_peak'] <= 476))
    return all(results)
...
where sortedness(<start>) >= 0.3333333333333333
where sortedness(<start>) <= 1.0
